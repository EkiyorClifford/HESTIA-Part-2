...
$tenant_user = $user->get_user_by('id', $usr_id);
...
$avg_price = $saved_count > 0
    ? array_sum(array_map(static fn($item) => (float) ($item['amount'] ?? 0), $my_wishlist)) / $saved_count
    : 0;
$states = array_values(array_unique(array_filter(array_map(static fn($item) => $item['state_name'] ?? '', $my_wishlist))));
$top_state = $states[0] ?? 'your preferred areas';
?>
<!DOCTYPE html>
<html lang="en">
<head>
